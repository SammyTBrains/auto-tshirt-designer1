...
import {
  ...
  DesignTransform,
  DesignHistoryItem,
} from "../pages/CustomDesign/types";

const PROMPT_TEMPLATES: PromptTemplates = {
  prefix: "",
  suffix:
    ", professional product photography, centered composition, high quality",
  negative:
    "distorted, blurry, bad art, watermark, text, deformed, out of frame, cropped, low quality",
};
...
export class DesignService {
  ...
  static async loadDesignHistory(): Promise<DesignHistoryItem[]> {
    try {
      const history = await apiService.get<DesignHistoryItem[]>(
        "/designs/history"
      );
      return history;
    } catch (error) {
  ...
  static async removeBackground(
    ...
    try {
      ...
      const response = await apiService.post<{ image: string }>(
        "/remove-background",
        {
          image: base64Data,
          transparency,
  ...
  static async adjustColorIntensity(
    ...
    try {
      ...
      const blobResponse = await apiService.postBlob(
        "/color-transparency",
        formData
      );
